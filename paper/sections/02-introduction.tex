...
\begin{itemize}
  ...
  \item Hercher~\cite{Hercher2023} proves that no Collatz \(m\)-cycle
        exists for \(m \leq 91\), where \(m\) denotes the number of
        local minima along the cycle (a parameter distinct from the
        cycle-length parameter \(k\) used in the present paper).
        Together with the earlier analytic bounds of Simons--de Weger
        type~\cite{SimonsDeWeger2005} and subsequent refinements, this
        is the sharpest explicit exclusion for small-\(m\) cycles
